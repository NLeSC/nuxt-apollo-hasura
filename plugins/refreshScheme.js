export default class RefreshScheme {
  ...
  _refreshLoop(interval) {
    ...
    if (typeof window !== 'undefined') {
      return setInterval(() => {
        return this._loginWithRefresh().catch((err) => {
  ...
  _loginWithUsername({ username, password, email }) {
    return this.axios({
  ...
  login({ data }) {
    ...
    this._logoutLocally()
    return this._loginWithUsername(data).then(() => {
      this._triggerRefresh()
  ...
  async fetchUser() {
    ...
    console.log('🎹', this._getToken(this.jwtKey))
    // Try to fetch user and then set
    console.log('🎹', await this.axios.get('http://localhost:8080/v1/graphql'))
    debugger
    return this.axios
      .get(this.options.endpoints.user.url)
      .then(({ data }) => {

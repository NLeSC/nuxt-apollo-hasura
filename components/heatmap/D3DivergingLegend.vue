<template>
  <div id="divergingLegendChart" ref="divergingLegendChart"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    drawChart() {
      ...
      d3.select('#divergingLegendChart').selectAll('*').remove()
      ...
      this.svg = d3.select('#divergingLegendChart').append('svg').attr('width', this.width).attr('height', this.height)
      ...
      this.drawDivergingLegend(this.heatmapLegend, 'features', [-2.5, 2.5], d3.interpolatePuOr, 6)

      this.pitchLegend = chartGroup
        .append('g')
        .attr('class', 'topicGroup')
        .attr('transform', 'translate(0, ' + this.barHeight * 7 + ')')
      this.drawDivergingLegend(this.pitchLegend, 'pitch', [0, 255], d3.interpolatePiYG, 6)
    ...
    drawDivergingLegend(d3Group, descriptor, extent, interpolator, ticks) {
      ...
      const divergingGradient = defs.append('linearGradient').attr('id', 'diverging-gradient-' + descriptor + '')
      ...
      d3Group
        .append('text')
        .attr('class', 'divergingDescription')
        .attr('x', 0)
      ...
      d3Group
        ...
        .style('fill', 'url(#diverging-gradient-' + descriptor + ')')
